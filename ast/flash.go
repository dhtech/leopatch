...
func (f *spiflash) cmd8(cmd int) {
	f.cs(0)
	defer f.cs(1)
	f.a.MustWrite8(FLASH_START, uint8(cmd&0xff))
}
...
func (f *spiflash) cmd8Read32(cmd int) uint32 {
	...
	defer f.cs(1)
	f.a.MustWrite8(FLASH_START, uint8(cmd&0xff))
	return f.a.MustRead32(FLASH_START)
}
...
func (f *spiflash) cmd8Read8(cmd int) uint8 {
	...
	defer f.cs(1)
	f.a.MustWrite8(FLASH_START, uint8(cmd&0xff))
	return f.a.MustRead8(FLASH_START)
}

func (a *Ast) SystemFlash() (Flash, error) {
	// Assume SPI flash
	// Reset CE0
	a.MustWrite32(CS0_CTRL, 0)
	a.MustWrite32(CS0_SEGMENT_ADDR, 0x48400000) // See manual for reset value
	a.MustWrite32(SPI_READ_TIMINGS, 0)
...
func (f *mx25l256) ReadAt(b []byte, off int64) (int, error) {
	...
	f.cs(0)
	defer f.cs(1)
	f.a.MustWrite8(FLASH_START, uint8(MX25_OP_FAST_READ&0xff))
	f.a.MustWrite8(FLASH_START, uint8(off>>24&0xff))
	f.a.MustWrite8(FLASH_START, uint8(off>>16&0xff))
	f.a.MustWrite8(FLASH_START, uint8(off>>8&0xff))
	f.a.MustWrite8(FLASH_START, uint8(off&0xff))
	f.a.MustWrite8(FLASH_START, 0) // 8 dummy cycles
	i := 0
	for ; i < l-3; i += 4 {
		d := f.a.MustRead32(FLASH_START)
		b[i] = byte(d & 0xff)
		b[i+1] = byte(d >> 8 & 0xff)
...
func (f *mx25l256) eraseBlock(b int32) {
	f.cmd8(MX25_OP_WREN)
	f.cs(0)
	f.a.MustWrite8(FLASH_START, uint8(MX25_OP_BLOCK_ERASE&0xff))
	f.a.MustWrite8(FLASH_START, uint8(b>>24&0xff))
	f.a.MustWrite8(FLASH_START, uint8(b>>16&0xff))
	f.a.MustWrite8(FLASH_START, uint8(0)) // Blocks are 64kb, lower 16b are 0
	f.a.MustWrite8(FLASH_START, uint8(0))
	f.cs(1)
...
func (f *mx25l256) programPage(p int32, d []byte) {
	...
	f.cmd8(MX25_OP_WREN)
	f.cs(0)
	f.a.MustWrite8(FLASH_START, uint8(MX25_OP_PAGE_PROGRAM&0xff))
	f.a.MustWrite8(FLASH_START, uint8(p>>24&0xff))
	f.a.MustWrite8(FLASH_START, uint8(p>>16&0xff))
	f.a.MustWrite8(FLASH_START, uint8(p>>8&0xff))
	f.a.MustWrite8(FLASH_START, uint8(0)) // Pages are 256 byte, lower 8b are 0
	for i := 0; i < len(d); i += 4 {
		v := uint32(d[i])
		v |= uint32(d[i+1]) << 8
		v |= uint32(d[i+2]) << 16
		v |= uint32(d[i+3]) << 24
		f.a.MustWrite32(FLASH_START, v)
	}
	f.cs(1)
